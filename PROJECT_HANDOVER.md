...
- `001.json` through `060.json` form the gap-free production prefix;
- stabilization batch **003–010** is complete;
- first regular 25-record batch **011–035** is complete;
- second regular 25-record batch **036–060** is complete;
- next record: **061**;
- next planned checkpoint: **061–085**.
...
`scripts/materialize_r15a_purananuru_batch.py` remains the semantic-schema materializer. `scripts/materialize_r15a_purananuru_batch_driver.py` is the range-aware orchestration layer: it selects the correct 50-record audit-control TSV and safely handles a reviewed spec that crosses an audit-part boundary. Neither script is an automatic classifier.
...
- first 25-record batch: **011–035**;
- second 25-record batch: **036–060**.

Next cadence:

- **061–085, 086–110, 111–135, ...** through 400.
...
The completed 036–060 batch preserves source terms and source-state distinctions including `பார்ப்பார்`, `இடையன்`, `யவனர்`, `முதுகுடி`, `பஞ்சவர்`, null canonical addressees where applicable, and the exact printed poet metadata for record 060.

## Puṟanāṉūṟu boundary

Review **061 onward** sequentially until all 400 records are complete.
...
Continue Puṟanāṉūṟu at record **061** and complete batch **061–085** sequentially using the source-first reviewed-spec → deterministic-materialization cadence. Publish the completed batch, validate the PR, and continue only if green.
